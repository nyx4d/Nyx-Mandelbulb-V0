...
<head>
    ...
    <title>Mandelbulb Exploration</title>
    <style>
...
// Configuration
const CONFIG = {
    MAX_ITERATIONS: 1000, // Change this value to speed up or slow down the evolution
    EVOLUTION_SPEED: 5.5  // Adjust this to change how fast the shape evolves
};
...
float mandelbulbSDF(vec3 pos) {
    ...
    float iterationFactor = sin(iteration * 0.01) * 0.5 + 1.5;
...
void main() {
    ...
    if(t > 0.0) {
        ...
        vec3 baseColor = vec3(
            sin(iteration * 0.02) * 0.5 + 0.5,
            cos(iteration * 0.015) * 0.5 + 0.5,
            sin(iteration * 0.01) * 0.5 + 0.5
        );
